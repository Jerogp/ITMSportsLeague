...
public class MatchLineupService : IMatchLineupService
{
    ...
    public async Task<MatchLineup> AddPlayerToLineupAsync(int matchId, MatchLineup matchLineup)
    {
        ...
        var player = await _matchValidationHelper.ValidatePlayerInMatchAsync(
        ...
        var createdWithDetails = await _matchLineupRepository.GetByIdWithDetailsAsync(created.Id);
    ...
    public async Task DeleteLineupAsync(int matchId, int lineupId)
    {
        ...
        if (lineup == null || lineup.MatchId != matchId)
            throw new KeyNotFoundException($"No se encontró la alineación con ID {lineupId} para el partido {matchId}");

        _logger.LogInformation(
            "Deleting lineup {LineupId} from match {MatchId}",
            lineupId,
            matchId);
